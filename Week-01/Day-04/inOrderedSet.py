...
def binary(ordered_list, number, high, low):
	if high >= low:
		mid = int (low + (high - low)/2)
		if ordered_list[mid] == number:
			return True
		elif ordered_list[mid] > number:
			return binary(ordered_list, number, mid-1, low)
		else:
			return binary(ordered_list, number, high, mid+1)

def contains(ordered_list, number): 
	if binary(ordered_list, number, len(ordered_list)-1, 0):
		return True
	else:
		return False
